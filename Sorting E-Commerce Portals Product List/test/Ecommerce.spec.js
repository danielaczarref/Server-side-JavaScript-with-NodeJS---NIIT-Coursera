...
const sortDataOnPrice = (fileContents, cb) => {
  try {
    // Parse retail_price as integer before sorting since it's a string in the input
    const parsedData = fileContents.map(item => ({
      ...item,
      retail_price: parseInt(item.retail_price, 10),
    }));

    // Use lodash.sortBy to sort the data by retail_price
    const sortedData = _.sortBy(parsedData, 'retail_price');

    // Convert retail_price back to string to match expected format
